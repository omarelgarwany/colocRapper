  ...
  for (i in 1:nrow(cmds)) {
    ...
    for (j in 1:num_traits) {
      ...
      if(tr_type=='quant') {
        ...
        tr_dat <- tr_dat %>% dplyr::filter( ph %in% all_of(tr_ph) )
      }
    ...
    all_summstat <- all_summstat %>% dplyr::filter(! (snp %in% all_summstat$snp[all_summstat$snp %>% duplicated()]))
    #Removing NA's
    all_summstat <- all_summstat %>% drop_na()

    
    #Removing SE=0
    all_summstat <- all_summstat %>% dplyr::filter(if_all(all_of(se_cols), ~ . > 0)) %>% dplyr::filter(if_all(all_of(se_cols), ~ . != Inf))  %>% dplyr::filter(if_all(all_of(beta_cols), ~ . != Inf))
